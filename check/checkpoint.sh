...
if [ -n "$file_location" ]; then
    ...
    # Construct the wget URL using the internal IP and filename
    wget_url="http://$INTERNAL_IP:8000/$filename"

    # Use wget to download the checkpoint file
    wget "$wget_url" -P "$folder_path"

    echo "Checkpoint file downloaded and saved: $filename"
else
    echo "File location not found in the output."
fi
